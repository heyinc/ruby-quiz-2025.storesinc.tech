...
import gameImage from "./images/game.sixel";
import ingtImage from "./images/ingt.sixel";
...
async function init() {
    ...
    await fetch(gameImage)
        .then(response => response.text())
        .then(text => {
            // @ts-ignore
            window.gameImages["game"] = text;
        });

    await fetch(ingtImage)
        .then(response => response.text())
        .then(text => {
            // @ts-ignore
            window.gameImages["ingt"] = text;
        });
